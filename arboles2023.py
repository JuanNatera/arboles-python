...
class ArbolBinario:
    ...
    def __posOrden(self, arbol:"ArbolBinario", visitados:list):
        ...
        return visitados
    #Zona de Influencia
    def buscarZona(self, nombre_ciudad:str) -> list:
        nodo_objetivo = []
        self.__preOrdenZona(self, nombre_ciudad, nodo_objetivo)
        if nodo_objetivo:
            return nodo_objetivo[0].preOrden()
        return None

    def __preOrdenZona(self, arbol:"ArbolBinario", ciudad_buscar:str, arbol_objetivo:list):        
        if arbol is not None:
            if isinstance(arbol.valor_nodo, Ciudad):
                if arbol.valor_nodo.nombre == ciudad_buscar:
                   arbol_objetivo.append(arbol)
                self.__preOrdenZona(arbol.hijo_izquierdo, ciudad_buscar, arbol_objetivo)                
                self.__preOrdenZona(arbol.hijo_derecho, ciudad_buscar, arbol_objetivo)        
...
class ArbolBinarioBusqueda:
    ...
    def buscarCiudadMaxPoblacion(self):        
        lista_ciudades = self.enOrden()
        if lista_ciudades:
            ciudad_max_pob = lista_ciudades[0].valor_nodo
            for nodo_actual in lista_ciudades:
                ciudad_actual = nodo_actual.valor_nodo
                print(ciudad_actual.anno_creacion)
                if ciudad_actual.poblacion > ciudad_max_pob.poblacion:
                    ciudad_max_pob = ciudad_actual
            return ciudad_max_pob
        return None